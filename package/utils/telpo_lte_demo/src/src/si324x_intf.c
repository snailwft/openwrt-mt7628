...
**
** $Id: si324x_intf.c 4755 2015-02-19 00:02:29Z nizajerk $
**
** Si324x_Intf.c
...
**
** File Description:
** This is the implementation file for the Si324x chipset driver.
**
*/
...
#ifndef DISABLE_LOOPMAN
int enableSi324xLoopman (proslicChanType *pProslic, int broadcast){
	uInt8 channel;
		ProSLIC_SetUserMode(pProslic,TRUE, broadcast);
	if (broadcast){
		channel = BROADCAST;
	}
	else{
...
int disableSi324xLoopman (proslicChanType *pProslic, int broadcast){
	...
	if (broadcast){
		...
		ProSLIC_SetUserMode (pProslic,TRUE, FALSE); /*make sure we are in user mode */
		channel = pProslic->channel;
	}
...
int Si324x_InitBroadcast (proslicChanType_ptr *pProslic){
    ...
    */
	if (pProslic[0]->deviceId->chipRev == 3) {/*rev d*/
		ProSLIC_LoadPatch_extended(pProslic[0],&RevDPatch,1);
    }
    ...
    */
	...
	ProSLIC_SetUserMode(pProslic[0],TRUE, FALSE);
...
#ifdef ENABLE_DEBUG
				if (pProslic->debugMode)
					LOGPRINT("%sLB Calibration timeout\n",LOGPRINT_PREFIX);
#endif
				pProslic->error = RC_CAL_TIMEOUT;
...
int Si324x_PCMSetup (proslicChanType *pProslic, int preset){
	...
	regTemp |= Si324x_PCM_Presets[preset].alaw_inv << 2;
