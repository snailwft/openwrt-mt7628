...
送忙音
*/
void tone_gen_busy(chanState *pState)
{
#if 0
	int ton = 350;
	int toff = 350;
	set_tonegen_osc_timers(pState,ton,toff);// 设定频率
#endif
	WriteReg(pProHW,pState->ProObj->channel,OCON,0x77);// inactive active all enable,
}	
...
送拨号音
*/
void send_tone_gen_dial(chanState *pState)
{
	WriteReg(pProHW,pState->ProObj->channel,OCON,0x11);// 写入11就能一直发出拨号音
}

/*
关拨号音
*/
void close_tone_gen_dial(chanState *pState)
{
