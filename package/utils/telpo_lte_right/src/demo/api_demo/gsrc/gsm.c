...
void gsm_cmgw()
{
	...
	char buf[100];
	//获取目的号码
	//AT+CMGW="13016054035" <CR> (TEXT MODE) 
...
error_exit:
	...
	simcom_disable(ports[0].ProObj); //模块直接断电
	//pthread_kill(tid, 0);
	//usleep(1000);
...
int gsm_recv()
{
	int res = 0;	
	int i = 0;
	//pthread_mutex_lock(&pcm_recv_mutex);
	res = uart_rev(fd, st_gsm_at.rx.rx_data, sizeof(st_gsm_at.rx.rx_data));
	//pthread_mutex_unlock(&pcm_recv_mutex);
	if (res > 0)
	{
...
void write_cmgl_information(const char *filename, const char *buf, int size)
{
	...
	//fseek(cmgl_short_sms, 0, SEEK_END);
...
void gsm_sms_handler()
{
	int i = 0;
#if 1
	if (st_gsm_at.at.order == AT_CMGL) 
	{
		write_cmgl_information(SHORT_SMS_FILE, st_gsm_at.rx.rx_data, st_gsm_at.rx.rx_length);
		st_gsm_at.at.state = AT_ANSWER_OK;
		memset(&st_gsm_at.rx, 0x00, sizeof(ST_AT_RX));
	}
	for (i = 0; i < st_gsm_at.rx.rx_length; i++)
