...
void  user_ring( chanState ports)
{
	...
	{
		case SUB_IDLE:		 	
			if (get_gsm_sub_state() == GSM_SUBSTATE_HANG_UP)
			{
				//sleep(3);
	...
	while (1)
	{
		...
		{			
			...
			case USER_STATE_IDLE:	
				...
				else if (get_gsm_sub_state() == GSM_SUBSTATE_RING)
				{					
					//sleep(1);
					answer_call = 1; //来电标志位
					...
					{
						//pthread_mutex_lock(&cid_mutex);
						send_cid_stream_new(ports, callnum);//发送来电显示 FSK模式
						//sendCIDStream(ports);
						sleep(2);						
