...
#endif
/*
DESKEY:加密秘钥    			固定长度15位数字字符
value:需要加密的缓冲区  	固定长度4位数字字符
Inlen:加密秘钥长度				15
Outbuff:加密的输出缓冲区
EncryFlag:加解密标志位 	1:加密		0:解密
*/
int encryp_codec(char *DESKEY,char *value,int Inlen,char *Outbuff,int EncryFlag)
{	
    int outlen;
    int i,j=0;
	 char temp[128];
	...
	{
		#ifdef _DBUG_
		printf("IMEI long is error\n");
		#endif
		dbug_log(__FILE__,__LINE__, "IMEI long is error\n");
		return -1;
	}
	for (i=0;i<Inlen;i++)
	{
		if (DESKEY[i]<'0'||DESKEY[i]>'0'+9)
		{
			#ifdef _DBUG_
			printf("IMEI is not full num\n");
			#endif
			dbug_log(__FILE__,__LINE__, "IMEI is not full num\n");
			return -1;
		}
	}
	#ifdef _DBUG_
	 ...
	 printf("value=%s\n",value);
	 #endif
	//15位IMEI取最后8位数字作为DESKEY
    outlen = crypt_des(DESKEY+7,value,strlen(value),temp,EncryFlag);
                
    for (i=0;i<outlen;i++)
    {
    	sprintf(Outbuff+2*i,"%02d",temp[i]&0xff);
		#ifdef _DBUG_
       printf("%02x",temp[i]&0xff);
		#endif
    }
	printf("\n");
	memset(&Outbuff[4],0,strlen(Outbuff));
