...
uInt16 API_Demo (){
  ...
  if(( (i>>3) & 7) != 6)
  {
...
#if 1
    	ProSLIC_ToneGenSetup(pSlic, 0); 		// 不同的国家有不同标准
    	ProSLIC_PCMSetup(pSlic, 0);
		ProSLIC_PCMTimeSlotSetup(pSlic, 0, 0);
    ...
    */
	...
	pthread_create(&gsm_init_deal, NULL, gsm_init_handler, (void *)&ports[0]);
