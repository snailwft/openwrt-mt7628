...
#if 1
int proslic_tone_gen_setup_new(proslicChanType_ptr hProslic,int preset,uInt32 amp1,uInt32 amp2)
{
	return Si3217x_ToneGenSetupnew(hProslic,preset,amp1,amp2);
}
#endif
...
static int ProSLIC_ReadInterruptsHelper(proslicChanType_ptr pProslic, uInt8 *regData, uInt8 numChannels)
{
	...
	uInt8 data = 0;
	if ((intsActive & 0x80) != 0)
	{
		data = ReadReg(pProslic, pProslic->channel, 3); 
		printf("read mstrstat data = %d\n", data);
		if (data &= 0xE0)
		{
			WriteReg(pProslic, pProslic->channel, 3, data); // 写入1清除
		}
	}
	if (interupt_time == 0)
	{
		interupt_time = 1000; // 10s读一次
		data = ReadReg(pProslic, pProslic->channel, 3); 
		printf("read mstrstat data = %d\n", data);	
		if (data != 31)
		{
			spiread_error_times++;
			if (spiread_error_times >= 2)
				goto errno_exit;
		}
		else 
			spiread_error_times = 0;
		data = ReadReg(pProslic, pProslic->channel, PROSLIC_REG_PCMMODE); 
		printf("read PCM_EN = %d\n", data);	
	}
#endif
    ...
    return RC_COMPLETE_NO_ERR;
errno_exit:
	dbug_log(__FILE__,__LINE__,"spi read error\n");
	//longjmp(env,1);	
	exit(1); 
}
