...
**
** $Id: si3217x_revc_intf.c 4667 2015-01-23 22:21:06Z nizajerk $
**
** SI3217X_RevC_Intf.c
 ...
 ramData ram_data;
    ...
    case INIT_SEQ_PRE_CAL:
        ...
        */
        ...
        {
            oi_sense_scale = 2; /* OITHRESH should be twice as large as user input when 0.2 ohm Rsense is used -LG, 07/18/14 */
        }
        else
        {
            oi_sense_scale = 1;
        ...
        ram_data = (oi_sense_scale*GCONF.i_oithresh > OITHRESH_OFFS)?(oi_sense_scale*GCONF.i_oithresh - OITHRESH_OFFS)/OITHRESH_SCALE:0L;
        ram_data *= BIT20LSB;
        WriteRAM(pProHW, pProslic->channel,SI3217X_COM_RAM_DCDC_OITHRESH,ram_data);
        
        ram_data = (oi_sense_scale*GCONF.i_oithresh_lo > OITHRESH_OFFS)?(oi_sense_scale*GCONF.i_oithresh_lo - OITHRESH_OFFS)/OITHRESH_SCALE:0L;
        ram_data *= BIT20LSB;
        WriteRAM(pProHW, pProslic->channel,SI3217X_REVC_RAM_DCDC_OITHRESH_LO,ram_data);
                
        ram_data = (oi_sense_scale*GCONF.i_oithresh_hi > OITHRESH_OFFS)?(oi_sense_scale*GCONF.i_oithresh_hi - OITHRESH_OFFS)/OITHRESH_SCALE:0L;
        ram_data *= BIT20LSB;
        ...
        */
		...
		WriteRAM(pProHW, pProslic->channel,SI3217X_COM_RAM_PWRSAVE_CTRL_LO,ram_data&0x07FFFFFFL); /* clear b27 */ 
        break;
...
ramData inv_off;
    ...
    case BO_DCDC_LCQC_6W:
        ...
        */
        WriteRAM(pProHW, pProslic->channel,SI3217X_REVC_RAM_DCDC_LIFT_EN,0x0L);      /* dcff disabled */
    ...
    case BO_DCDC_BUCK_BOOST:
        ...
        */
		...
		WriteRAM(pProHW, pProslic->channel,SI3217X_REVC_RAM_DCDC_LIFT_EN,0x100000L); /* dcff enabled */
        break;
