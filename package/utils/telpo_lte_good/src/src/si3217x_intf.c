...
**
** $Id: si3217x_intf.c 4793 2015-03-05 17:24:26Z nizajerk $
**
** SI3217X_Intf.c
...
int Si3217x_Init_with_Options (proslicChanType_ptr *pProslic, int size, initOptionsType init_opt)
{
  ...
  if( (init_opt == INIT_REINIT) || (init_opt == INIT_SOFTRESET) )
  {
    ...
    for(k = 0; k < j; k++)
    {
      pProslic[k]->scratch = pProslic[k]->ReadRegX(pProslic[k]->pProHWX,pProslic[k]->channel,SI3217X_COM_REG_LINEFEED);
      Si3217x_PowerDownConverter(pProslic[k]);
    }
    ...
    if(init_opt == INIT_REINIT)
    {
      for(k= 0; k  < j; k++)
      {
        pProslic[k]->WriteRegX(pProslic[k]->pProHWX,pProslic[k]->channel,SI3217X_COM_REG_RESET,0x01); /* device ch 0 */
      }
    }
    ...
    */
    for (k=0;k<size;k++)
    {
      if ((pProslic[k]->channelEnable)&&(pProslic[k]->channelType == PROSLIC))
      {
        ...
        {
          uInt32 patchData;
          patchData = pProslic[k]->ReadRAMX(pProslic[k]->pProHWX, pProslic[k]->channel,SI3217X_COM_RAM_PATCHID);
  ...
  */
  ...
  { 
    ...
    pProslic[k]->WriteRAMX(pProslic[k]->pProHWX,pProslic[k]->channel,SI3217X_COM_RAM_IRING_LIM,SI3217X_IRING_LIM_MAX);
  }
  ...
  */
  ...
  { 
    ...
    if((init_opt == INIT_REINIT) || (init_opt == INIT_SOFTRESET))
    {
      pProslic[k]->WriteRegX(pProslic[k]->pProHWX,pProslic[k]->channel,SI3217X_COM_REG_LINEFEED,pProslic[k]->scratch);
    }
  }
...
int Si3217x_RingSetup (proslicChanType *pProslic, int preset){
    ...
    */
    ...
    {
        WriteReg(pProHW,pProslic->channel,SI3217X_COM_REG_RINGCON,(0x80|Si3217x_Ring_Presets[preset].ringcon) & ~(0x40));
...
int Si3217x_ToneGenSetupnew (proslicChanType *pProslic, int preset,uInt32 amp1,uInt32 amp2)
{
	 if (pProslic->channelType != PROSLIC) {
        return RC_CHANNEL_TYPE_ERR;
    }

    WriteRAM(pProHW,pProslic->channel,SI3217X_COM_RAM_OSC1FREQ,0x05e00000L);// 950hz
    WriteRAM(pProHW,pProslic->channel,SI3217X_COM_RAM_OSC1AMP,amp1);
    ...
    WriteRAM(pProHW,pProslic->channel,SI3217X_COM_RAM_OSC2FREQ,0x05e00000L);// 950hz
    WriteRAM(pProHW,pProslic->channel,SI3217X_COM_RAM_OSC2AMP,amp2);
...
int Si3217x_FSKSetup (proslicChanType *pProslic, int preset){
    ...
    WriteReg(pProHW,pProslic->channel,SI3217X_COM_REG_FSKDEPTH,0x08); /* Clear Buffer */
